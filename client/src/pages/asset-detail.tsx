...
export default function AssetDetail() {
  ...
  // Default to BTC if no symbol, and ensure it's treated as a pair
  const rawSymbol = params?.symbol ? decodeURIComponent(params.symbol) : "BTC";
  
  // Handle pair display:
  // If raw is "BTC", display is "BTC", pair is "BTC/USDT"
  // If raw is "BTC/USDT", display is "BTC", pair is "BTC/USDT"
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      setIsLoading(true);
      try {
        // Use the provided 30-day URL pattern as default
        // We'll use a placeholder {id} to be replaced by the actual coin ID
        const defaultApiUrl = "https://api.coingecko.com/api/v3/coins/{id}/market_chart?vs_currency=usd&days=30&x_cg_demo_api_key=CG-7Rc5Jh3xjgp1MT5J9vG5BsSk";
        ...
        // Default to lowercase symbol if not found in map (often works for many coins)
        const coinId = symbolToId[displaySymbol.toUpperCase()] || displaySymbol.toLowerCase();
        
        // Replace {id} in the template
        ...
        const formattedData = prices.map((item: [number, number]) => ({
          // Format time based on the timeframe - for 30 days, showing Date + Time is good
        ...
        // Downsample to ~100 points for performance if needed
        // For 30 days (hourly data = ~720 points), taking every 6th point gives ~120 points
        const step = Math.ceil(formattedData.length / 100); 
        ...
        if (prices.length > 0) {
          ...
          // Calculate change based on the first point of the chart (30 days ago in this case)
          // Or realistically for "24h change" we should look at the point from 24h ago
          // For this demo, we'll just compare start vs end of the chart range or use a mock 24h calculation
          const firstPrice = prices[0][1];
          const chartChange = ((latestPrice - firstPrice) / firstPrice) * 100;
          
          setAssetInfo({
            price: latestPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
            change: `${chartChange > 0 ? '+' : ''}${chartChange.toFixed(2)}%`,
            isUp: chartChange > 0,
